...
def parse():
    ...
    parser.add_argument('-o', '--out', type=str, default=None,
                help='set output directory for generated Prolog/Popper files \
                    (default: dataset stem in current directory)')
    parser.add_argument('--keep-prolog-files', action='store_true',
                help='keep generated Prolog/Popper files (bias.pl, bk.pl, exs.pl, dynamic.pl) \
                    instead of deleting them at the end')
...
def main(run_as_package=False,args={}):
    ...
    out_dir = args.out
    keep_prolog_files = args.keep_prolog_files

    base_out_path = str(Path(out_dir) / Path(table_path).stem) if out_dir else Path(table_path).stem
    ...
    random_n, rebinds = parse_table(table_path, int_cols, grounded_ops, sample_size, categorical, base_out_path)
    ...
    if categorical:
        for category in categories:
            out_paths.append(base_out_path + "-" + category)
    else:
        out_paths = [ base_out_path ]
    ...
    for out_path in out_paths:
        ...
        # Remove generated files unless explicitly requested otherwise
        if not keep_prolog_files:
            try:
