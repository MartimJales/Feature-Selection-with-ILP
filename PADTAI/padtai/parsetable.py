...
def is_number(str):
    ...
    # Guard against None or empty strings which would raise on indexing
    if str is None:
        return False
    s = str.strip()
    if s == "":
    ...
    # Keep original heuristic: accept integers and floats (including negative)
    return s.replace('.', '', 1).isdigit() or (s[0] == '-' and s[1:].replace('.', '', 1).isdigit())
