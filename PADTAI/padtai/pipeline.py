        ...
        try:
            if target_mode:
                ...
                category = next(
                    ...
                    if any(attr.endswith(category) for attr in list(map(lambda el: el.split('_', 2)[-1] if el.startswith("attr") else el, rule_formatted.split(','))))
                )
        except StopIteration:
            # Fallbacks: prefer head_formatted if it is a known category,
            # else pick first category if available, else None
            if head_formatted in categories:
                category = head_formatted
            elif categories:
